...
public class MultiChatServerHelper extends Thread{
	...
	public MultiChatServerHelper(Socket socket, DefaultListModel<String> dlm,
			int cnt, JFrame jf, List<MultiChatServerHelper> list, JScrollPane jsp) {
		someClient=socket;
		this.dlm=dlm;
		this.cnt=cnt;
		this.jf=jf;
		...
		try {
			...
			//서버창에 접속 메세지 출력
			dlm.addElement("["+someClient.getInetAddress()+":"+
					someClient.getLocalPort()+" / "+nick+"] 님이 접속하셨습니다.");
